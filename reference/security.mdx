---
title: "Security"
description: "OpenPawz is designed with defense-in-depth: 20+ security layers protect against prompt injection, data exfiltration, SSRF, MITM attacks, memory forensics, agent fixation, inter-agent manipulation, and unauthorized actions."
---
...
| `lock-screen` | `PURPOSE_LOCK_SCREEN` | Argon2id hashed passphrase (legacy SHA-256 supported) |
...
| `scc-signing` | `PURPOSE_SCC_SIGNING` | Session Continuity Certificate signing key |
| `oauth:{service}` | (dynamic) | Per-service encrypted OAuth tokens |

All in-memory key material is wrapped in `Zeroizing<String>` (via the `zeroize` crate) so secrets are securely overwritten with zeroes when dropped or replaced — preventing key material from lingering in freed heap memory.
...
| Edge HMAC | `engram-edge-key-v1` | Keyed HMAC tokenization of edge types |
...
The AES-256-GCM skill layer protects skill credentials stored in the `skill_credentials` table. The AES-256-GCM database layer protects other sensitive fields across the database. Both derive their keys from the unified OS keychain vault (`openpawz`) using separate purpose keys.
:::
...
- **Level 2+:** Hard override — "USER COMMAND: The user has told you {N} times to stop"
...
The lock screen passphrase is hashed with **Argon2id** (memory-hard, timing-attack resistant) before storage. Legacy SHA-256 hashes are transparently supported for backward compatibility. Argon2id verification is constant-time by design; the legacy SHA-256 fallback uses `subtle::ConstantTimeEq` to prevent timing side-channel attacks.
...
| Comparison | Constant-time via `subtle::ConstantTimeEq` — prevents timing side-channels |
...
The audit chain proves integrity within a single session, but does not chain sessions together. **Session Continuity Certificates** solve the *Ghost Agent Problem* — the risk that an attacker could swap model weights or agent objectives between sessions while reusing the same OS keychain credentials.

At every engine startup, a signed SCC is issued and stored in the `session_continuity_certs` table alongside the audit log.

### What the SCC commits to

| Field | Purpose |
|-------|---------|
| `model_id` | Which LLM model is configured as the default |
| `capability_hash` | SHA-256 of the sorted Tauri capability/permission set |
| `memory_hash` | Signature of the latest audit log entry (anchors to the audit chain state) |
| `prior_cert_hash` | HMAC of the previous SCC (or genesis `000…` on first-ever boot) |

### How it works

1. Engine starts → `startup_housekeeping()` runs DB cleanup
2. Reads `config.default_model` for the active model ID
3. Hashes the Tauri capability set (SHA-256 of sorted permission strings)
4. Reads the audit chain tip as `memory_hash`
5. Chains to the prior SCC's signature (forward-chain linkage)
6. HMAC-signs all fields with the dedicated `scc-signing` vault key
7. Stores the certificate in `session_continuity_certs`

### Verification

`scc::verify_chain()` walks all certificates from genesis to latest, verifying:
- **Forward chain linkage** — each cert's `prior_cert_hash` must match the previous cert's signature
- **Signature integrity** — recomputed HMAC must match the stored signature
- **Constant-time comparison** — all checks use `subtle::ConstantTimeEq`

Any gap, model substitution, or forgery breaks the chain with a descriptive error identifying the exact certificate where continuity was lost.

:::tip Cross-session identity attestation
The SCC chain lets you cryptographically prove that session N was started by the same agent configuration as session N-1. This is critical for long-running autonomous agents that accumulate reputation and make persistent decisions across many sessions.
:::
...
MCP tools whose names contain dangerous operation keywords are blocked at the name level:
