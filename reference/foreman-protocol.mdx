...
*Invented by Eli Bury for OpenPawz. Open source (MIT).*
...
**Traditional tool execution:** The LLM must have the tool's schema in its context to know how to call it. If you have 25,000 tools, you need 25,000 schemas in context — impossible.

**With MCP:** The Foreman connects to the MCP server and asks *"What tools do you have?"* The server responds with complete schemas for every available tool — parameter names, types, descriptions, required fields, examples. The Foreman uses these schemas to format its call correctly.

No pre-training. No static configuration. No context window overflow.

This means:
- **Any new n8n community node is instantly accessible** — install the npm package, the MCP server exposes it, the Foreman can read from or write to it
...
- **Reads are as natural as writes** — querying a database and sending a Slack message go through the same path
...
Together, they enable an agent to discover and execute any of 25,000+ integrations at near-zero cost — something no other AI agent platform achieves.

---

## License & Attribution

The Foreman Protocol was invented by **Eli Bury** as part of **OpenPawz** and is released under the **MIT License**. You are free to use, modify, and redistribute this technique in any project, commercial or otherwise. Attribution is appreciated but not required.

If you reference this work in academic papers or technical writing:

> Bury, E. (2025). "The Foreman Protocol: Low-Cost Tool Execution via Worker Models and Self-Describing MCP." OpenPawz. https://github.com/OpenPawz/openpawz
