...
Auth uses `Authorization: Bearer <mcp-token>`. The MCP token is a JWT
obtained via `POST /rest/mcp/api-key` (requires session auth from owner
login). Response: `{ "data": { "apiKey": "<jwt>", "audience": "mcp-server-api" } }`.
This is NOT the same as `N8N_API_KEY`. The `register_n8n()` function tries
MCP token + Streamable HTTP first, then API key fallbacks.

The flow:
```
n8n starts → owner account created (POST /rest/owner/setup)
           → MCP access enabled (PATCH /rest/mcp/settings)
           → MCP API key created (POST /rest/mcp/api-key)
...
`***REMOVED***`. The owner account is separate from the API key — the API
