...
function renderGoogleOAuthSection(skill: EngineSkillStatus): string {
  const hasRefreshToken = skill.configured_credentials.includes('GOOGLE_REFRESH_TOKEN');
  ...
  if (hasRefreshToken) {
    return `<div class="skill-cred-section">
      <div class="skill-section-title">${msIcon('cloud_done')} Google Account</div>
      <div class="skill-google-connected">
        <div class="skill-google-status">
          <span class="skill-google-check">${msIcon('check_circle')}</span>
          <span class="skill-google-email" id="google-connected-email">Connected — loading...</span>
        </div>
        <button class="btn btn-ghost btn-sm skill-google-disconnect" id="google-disconnect-btn">
          ${msIcon('link_off')} Disconnect
        </button>
      </div>
    </div>`;
  }
  ...
  if (hasServiceAccount) {
    return `<div class="skill-cred-section">
      <div class="skill-section-title">${msIcon('key')} Service Account (Enterprise)</div>
      <div class="skill-google-connected">
        <div class="skill-google-status">
          <span class="skill-google-check">${msIcon('check_circle')}</span>
          <span>Connected via service account</span>
        </div>
        <button class="btn btn-ghost btn-sm skill-google-disconnect" id="google-sa-revoke-btn">
          ${msIcon('close')} Remove
        </button>
      </div>
      <p class="skill-google-hint">Want to switch to the simpler OAuth flow? Remove the service account and use "Connect with Google" instead.</p>
    </div>`;
  }
  ...
  return `<div class="skill-cred-section">
    ...
    <div class="skill-google-setup">
      <p class="skill-google-hint" style="margin-bottom: 1rem;">
        To connect Pawz to Google, you need an OAuth Client ID from Google Cloud Console.<br/>
        <a href="https://console.cloud.google.com/apis/credentials/oauthclient" target="_blank" class="skill-google-link">Create one here</a> →
        choose <strong>"Desktop app"</strong> → copy the Client ID and Secret below.
      </p>
      ...
      <button class="btn btn-primary skill-google-connect" id="google-connect-btn"
        ${oauthReady ? '' : 'disabled'} style="margin-top: 0.75rem; width: 100%;">
        ${msIcon('login')} Connect with Google
      </button>
      ${!oauthReady ? '<p class="skill-google-hint">Set the Client ID and Secret above first, then click Connect.</p>' : ''}
    </div>
...
function renderSingleCredRow(
  skill: EngineSkillStatus,
  key: string,
  label: string,
  description: string,
  isSet: boolean,
  ...
  return `
    <div class="skill-cred-row">
      <div class="skill-cred-label">
        <label class="skill-cred-name">${escHtml(label)} <span class="skill-required">*</span></label>
        <span class="skill-cred-desc">${escHtml(description)}</span>
      </div>
      <div class="skill-cred-field">
        <input type="password" class="form-input skill-cred-input"
          data-skill="${escHtml(skill.id)}" data-key="${escHtml(key)}"
          placeholder="${isSet ? '••••••••' : escHtml(placeholder)}" />
        <button class="btn btn-ghost btn-sm skill-cred-save"
          data-skill="${escHtml(skill.id)}" data-key="${escHtml(key)}">
          ${isSet ? 'Update' : 'Set'}
        </button>
        ${isSet ? `<button class="btn btn-ghost btn-sm skill-cred-delete" data-skill="${escHtml(skill.id)}" data-key="${escHtml(key)}">${msIcon('close')}</button>` : ''}
      </div>
...
function renderCredentialFields(skill: EngineSkillStatus): string {
  // ── Google Workspace: show OAuth connect button as primary ──
  if (skill.id === 'google_workspace') {
    return renderGoogleOAuthSection(skill);
  }
...
export function bindSkillEvents(): void {
  ...
  if (googleConnectBtn) {
    googleConnectBtn.addEventListener('click', async () => {
      googleConnectBtn.setAttribute('disabled', 'true');
      googleConnectBtn.textContent = 'Connecting…';
      try {
        const email = await pawEngine.googleOAuthConnect();
        showToast(`Connected as ${email}`, 'success');
        await reload();
      } catch (err) {
        showToast(`Google OAuth failed: ${err}`, 'error');
        googleConnectBtn.removeAttribute('disabled');
