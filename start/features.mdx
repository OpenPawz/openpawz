...
<Columns cols={2}>
  ...
  <Card title="25,000+ integrations">
    400+ native integrations compiled into the binary, plus 25,000+ community node types via the MCP Bridge to n8n. Agents discover and install integrations at runtime, on demand.
  </Card>
...
Plus **25,000+ community node types** via the embedded n8n engine and MCP Bridge. Agents discover, auto-install, and execute any n8n community node at runtime — with zero configuration and zero cloud cost (local Ollama worker).
...
<Tabs>
  ...
  <Tab title="MCP">
    **Model Context Protocol.** Connect to any MCP server — stdio or SSE transport — for unlimited external tools. Databases, APIs, file systems, and more.
  </Tab>
...
**Unique to OpenPawz.** A visual, drag-and-drop workflow editor for building agent pipelines without writing code. Connect nodes — triggers, agents, tools, conditions, outputs — to create complex automation flows visually.

→ Deep dive: [Projects guide](/guides/projects)
...
<Columns cols={2}>
  ...
  <Card title="530 automated tests">
...
| **Built-in integrations** | 400+ | ~20 | ~15 | Code-defined | Framework only |
...
**OpenPawz is the first AI agent operating system** that combines 400+ integrations, multi-agent orchestration, 11 channels, DeFi trading, memory, voice, a node editor, and a three-tier marketplace in a single, native, open-source desktop application. Nothing else comes close.
