...
#[tauri::command]
pub fn engine_tasks_list(state: State<'_, EngineState>) -> Result<Vec<Task>, String> {
    state.store.list_tasks().map_err(|e| e.to_string())
}

#[tauri::command]
pub fn engine_task_create(state: State<'_, EngineState>, task: Task) -> Result<(), String> {
...
#[tauri::command]
pub fn engine_task_update(state: State<'_, EngineState>, task: Task) -> Result<(), String> {
    info!("[engine] Updating task: {} status={}", task.id, task.status);
    state.store.update_task(&task).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn engine_task_delete(state: State<'_, EngineState>, task_id: String) -> Result<(), String> {
...
pub fn engine_task_activity(
    ...
    match task_id {
        Some(id) => state
            .store
            .list_task_activity(&id, limit)
            .map_err(|e| e.to_string()),
        None => state
            .store
            .list_all_activity(limit)
            .map_err(|e| e.to_string()),
    }
}
...
pub fn engine_task_set_agents(
    ...
    info!(
        "[engine] Setting {} agent(s) for task {}",
        agents.len(),
        task_id
    );
...
#[tauri::command]
pub fn engine_tasks_cron_tick(state: State<'_, EngineState>) -> Result<Vec<String>, String> {
    ...
    for task in due {
        ...
        state
            .store
            .update_task_cron_run(&task.id, &now.to_rfc3339(), next.as_deref())?;
        let aid = uuid::Uuid::new_v4().to_string();
        state.store.add_task_activity(
            &aid,
            &task.id,
