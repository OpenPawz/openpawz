...
pub fn engine_trading_history(
    ...
    state
        .store
        .list_trades(limit.unwrap_or(100))
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn engine_trading_summary(state: State<'_, EngineState>) -> Result<serde_json::Value, String> {
    state.store.daily_trade_summary().map_err(|e| e.to_string())
}

#[tauri::command]
pub fn engine_trading_policy_get(state: State<'_, EngineState>) -> Result<TradingPolicy, String> {
...
pub fn engine_trading_policy_set(
    ...
    state
        .store
        .set_config("trading_policy", &json)
        .map_err(|e| e.to_string())
}
...
pub fn engine_positions_list(
    ...
    state
        .store
        .list_positions(status.as_deref())
        .map_err(|e| e.to_string())
}

#[tauri::command]
pub fn engine_position_close(state: State<'_, EngineState>, id: String) -> Result<(), String> {
    info!("[engine] Manually closing position {}", id);
    state
        .store
        .close_position(&id, "closed_manual", None)
        .map_err(|e| e.to_string())
}
...
pub fn engine_position_update_targets(
    ...
    info!(
        "[engine] Updating position {} targets: SL={:.0}%, TP={:.1}x",
        id,
        stop_loss_pct * 100.0,
        take_profit_pct
    );
    state
        .store
        .update_position_targets(&id, stop_loss_pct, take_profit_pct)
        .map_err(|e| e.to_string())
}
