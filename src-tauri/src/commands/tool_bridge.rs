...
#[tauri::command]
pub fn engine_tools_remap(raw_names: Vec<String>) -> Result<Vec<RemappedTool>, String> {
...
pub fn engine_tools_by_service(
    ...
    for tool in tools {
        let entry = groups
            .entry(tool.service.clone())
            .or_insert_with(|| ServiceToolSet {
...
pub fn engine_tools_set_agent_service(
    ...
    if let Some(existing) = assignments.iter_mut().find(|a| a.agent_id == agent_id) {
        ...
        assignments.push(AgentToolAssignment { agent_id, services });
    }
