...
fn build_skill_status(
    ...
    let missing_creds: Vec<String> = def
    ...
    let missing_bins: Vec<String> = def
        .required_binaries
        .iter()
        .filter(|bin| {
    ...
    let missing_envs: Vec<String> = def
        .required_env_vars
        .iter()
    ...
    let is_ready =
        enabled && missing_creds.is_empty() && missing_bins.is_empty() && missing_envs.is_empty();

    let custom_instr = store
