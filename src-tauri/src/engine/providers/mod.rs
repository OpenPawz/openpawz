...
// Anthropic and Google providers + AnyProvider factory.
// OpenAI-compatible provider lives in providers/openai.rs.

use crate::engine::types::*;
use crate::atoms::traits::AiProvider;
// Bring shared retry utilities from the openai module into this scope
// so AnthropicProvider and GoogleProvider can use them unchanged.
use self::openai::{MAX_RETRIES, is_retryable_status, retry_delay, parse_retry_after};
...
impl AnyProvider {
    ...
    pub async fn chat_stream(
        ...
        match self {
            // OpenAiProvider now implements AiProvider — bridge ProviderError → String
            AnyProvider::OpenAi(p) => p.chat_stream(messages, tools, model, temperature).await.map_err(|e| e.to_string()),
