...
use super::helpers::{amount_to_lamports, lamports_to_amount, parse_solana_keypair, resolve_token};
use super::rpc::{check_tx_confirmation, get_sol_balance, get_token_accounts, rpc_call};
use super::transaction::{build_solana_transaction, derive_ata, sign_solana_transaction};
...
pub async fn execute_sol_transfer(
    ...
    let private_key_b58 = creds
        .get("SOLANA_PRIVATE_KEY")
        .ok_or("No Solana private key.")?;

    let currency = args["currency"]
        .as_str()
        .ok_or("sol_transfer: missing 'currency'")?;
    let amount_str = args["amount"]
        .as_str()
        .ok_or("sol_transfer: missing 'amount'")?;
    let to_address = args["to_address"]
        .as_str()
        .ok_or("sol_transfer: missing 'to_address'")?;
    let _reason = args["reason"].as_str().unwrap_or("transfer");

    // Validate recipient address
    let to_bytes_vec = bs58::decode(to_address)
        .into_vec()
    ...
    let sender_pubkey_vec = bs58::decode(wallet)
        .into_vec()
        .map_err(|e| EngineError::Other(e.to_string()))?;
    ...
    let tx_sig = if is_sol {
        ...
        if balance < lamports + 5000 {
            ...
            return Err(format!(
                "Insufficient SOL balance. Have: {} SOL, need: {} SOL + fees",
        ...
        let signed_b64 =
            base64::Engine::encode(&base64::engine::general_purpose::STANDARD, &signed_tx);
        ...
        let decimals = if token_decimals == 0 {
            9
        } else {
            token_decimals
        };
        ...
        let mint_bytes_vec = bs58::decode(&mint_addr)
            .into_vec()
            .map_err(|e| EngineError::Other(e.to_string()))?;
        ...
        let sender_token = token_accounts.iter().find(|(m, _, _, _)| m == &mint_addr);
        match sender_token {
            None => {
                return Err(format!("No {} token account found in wallet", currency_upper).into())
            }
            Some((_, amt, _, _)) if *amt < raw_amount => {
                return Err(format!(
                    "Insufficient {} balance. Have: {}, need: {}",
                    currency_upper,
                    lamports_to_amount(*amt, decimals),
                    amount_str
                )
                .into());
            }
        ...
        let sender_ata_bytes = bs58::decode(&sender_ata_addr)
            .into_vec()
            .map_err(|e| EngineError::Other(e.to_string()))?;
        ...
        .await
        .map(|r| r.get("value").is_some_and(|v| !v.is_null()))
        .unwrap_or(false);
    ...
    info!(
        "[sol_dex] Transfer {} {} → {} | tx: {}",
        amount_str, currency_upper, to_address, tx_sig
    );
