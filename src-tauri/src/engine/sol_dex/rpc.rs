...
use super::constants::{TOKEN_2022_PROGRAM_ID, TOKEN_PROGRAM_ID};
...
pub(crate) async fn get_token_accounts(
    rpc_url: &str,
    wallet: &str,
) -> EngineResult<Vec<(String, u64, u8, String)>> {
                ...
                for acct in accounts {
                    ...
                    if let Some(info) = parsed {
                        ...
                        if let Some(ta) = token_amount {
                            let amount_str =
                                ta.get("amount").and_then(|v| v.as_str()).unwrap_or("0");
                            let amount: u64 = amount_str.parse().unwrap_or(0);
                            let decimals =
                                ta.get("decimals").and_then(|v| v.as_u64()).unwrap_or(0) as u8;
                            let pubkey = acct
                                .get("pubkey")
                                .and_then(|v| v.as_str())
                                .unwrap_or("")
    ...
    match get_mint_info(rpc_url, mint).await {
        Ok(info) => {
            if let Some(d) = info.get("decimals").and_then(|v| v.as_u64()) {
                info!(
                    "[sol_dex] Resolved on-chain decimals for {}: {}",
                    &mint[..8.min(mint.len())],
                    d
                );
                d as u8
                ...
                info!(
                    "[sol_dex] No decimals in mint info for {}, defaulting to 9",
                    &mint[..8.min(mint.len())]
                );
                9
            }
        ...
        Err(e) => {
            info!(
                "[sol_dex] Failed to get mint info for {}: {}, defaulting to 9",
                &mint[..8.min(mint.len())],
                e
            );
            9
