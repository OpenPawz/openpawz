...
pub async fn execute_dex_token_info(
    ...
    if !addr_clean.starts_with("0x") || addr_clean.len() != 42 {
        return Err(format!(
            "Invalid contract address format: '{}'. Must be 0x + 40 hex chars.",
            addr_clean
        )
        .into());
    }
    ...
    match eth_call(rpc_url, addr_clean, &encode_name()).await {
        ...
        Err(_) => {
            output.push_str("  Name: Could not read (non-standard contract)\n");
        }
    }
    ...
    match eth_call(rpc_url, addr_clean, &encode_symbol()).await {
        ...
        Err(_) => {
            output.push_str("  Symbol: Could not read\n");
        }
    }
    ...
    match eth_call(rpc_url, addr_clean, &encode_decimals()).await {
        ...
        Err(_) => {
            output.push_str("  Decimals: 18 (assumed)\n");
        }
    }
    ...
    match eth_call(rpc_url, addr_clean, &encode_total_supply()).await {
        ...
        Err(_) => {
            output.push_str("  Total Supply: Could not read\n");
        }
    }
    ...
    match eth_call(rpc_url, addr_clean, &encode_owner()).await {
            ...
            if bytes.len() >= 32 {
                ...
                if owner_addr == zero_addr {
                    ...
                    output.push_str(&format!(
                        "  Owner: {} [WARNING: not renounced — owner can modify contract]\n",
                        owner_hex
                    ));
                }
            }
        ...
        Err(_) => {
            output.push_str("  Owner: No owner() function (may be immutable) [SAFE]\n");
        }
    }
    ...
    let code_result = rpc_call(
        rpc_url,
        "eth_getCode",
        serde_json::json!([addr_clean, "latest"]),
    )
    .await;
    ...
    for fee in &[3000u32, 10000, 500, 100] {
        let quote_data =
            encode_quote_exact_input_single(&weth_addr_bytes, &token_bytes, &tiny_amount, *fee);
        ...
        if let Ok(result) = eth_call(rpc_url, UNISWAP_QUOTER_V2, &quote_data).await {
            ...
            if result_bytes.len() >= 32 {
                let amount_out: [u8; 32] = result_bytes[..32]
                    .try_into()
                ...
                if let Ok(out_amount) = raw_to_amount(&out_hex, token_decimals) {
                    output.push_str(&format!(
                        "    Uniswap V3 pool found ({}% fee tier) [OK]\n",
                        *fee as f64 / 10000.0
                    ));
                        ...
                        if let Ok(sell_u256) = parse_u256_decimal(&sell_raw) {
                            ...
                            match eth_call(rpc_url, UNISWAP_QUOTER_V2, &reverse_quote).await {
                                Ok(rev_result) => {
                                    ...
                                    if rev_bytes.len() >= 32 {
                                        let rev_out: [u8; 32] = rev_bytes[..32]
                                            .try_into()
                                            .map_err(|_| "Byte conversion failed")?;
                                        ...
                                        if let Ok(rev_amount) = raw_to_amount(&rev_hex, 18) {
                                            ...
                                            let round_trip_loss =
                                                ((original - rev_f) / original * 100.0).abs();
                                            output.push_str(&format!(
                                                "    SELL quote works: {} tokens -> {} WETH [OK]\n",
                                                out_amount, rev_amount
                                            ));
                                            output.push_str(&format!(
                                                "    Round-trip loss: {:.2}% (fees + slippage)\n",
                                                round_trip_loss
                                            ));
                                            if round_trip_loss > 50.0 {
                                                ...
                                                output.push_str(
                                                    "    Normal fee range — not a honeypot [OK]\n",
                                                );
                                            }
                                        }
                                    }
...
pub async fn execute_dex_check_token(
    ...
    match code_result {
        ...
        Err(e) => {
            output.push_str(&format!("[WARNING] Could not check contract code: {}\n", e));
        }
    }

    // Check 2: ERC-20 standard compliance
    let has_name = eth_call(rpc_url, addr_clean, &encode_name()).await.is_ok();
    let has_symbol = eth_call(rpc_url, addr_clean, &encode_symbol())
        .await
        .is_ok();
    let has_decimals = eth_call(rpc_url, addr_clean, &encode_decimals())
        .await
        .is_ok();
    let has_supply = eth_call(rpc_url, addr_clean, &encode_total_supply())
        .await
        .is_ok();
        ...
        Ok(result) => {
            ...
            if bytes.len() >= 32 {
                ...
                if owner_addr == [0u8; 20] {
                    ...
                    let owner_hex = eip55_checksum(owner_addr);
                    output.push_str(&format!(
                        "[WARNING] Owner: {} — can potentially modify contract\n",
                        owner_hex
                    ));
                    risk_score += 3;
    ...
    for fee in &[3000u32, 10000, 500, 100] {
        let buy_quote =
            encode_quote_exact_input_single(&weth_bytes, &token_bytes, &tiny_amount, *fee);
        if let Ok(result) = eth_call(rpc_url, UNISWAP_QUOTER_V2, &buy_quote).await {
            let result_bytes = hex_decode(&result).unwrap_or_default();
            if result_bytes.len() >= 32 {
                let out: [u8; 32] = result_bytes[..32]
                    .try_into()
                    .map_err(|_| "Byte conversion failed")?;
                ...
                if let Ok(amount) = raw_to_amount(&out_hex, token_decimals) {
                    can_buy = true;
                    output.push_str(&format!(
                        "  [OK] BUY works: 0.001 WETH -> {} tokens ({}% fee)\n",
                        amount,
                        *fee as f64 / 10000.0
                    ));
                    ...
                    if let Ok(sell_raw) = amount_to_raw(&amount, token_decimals) {
                        if let Ok(sell_u256) = parse_u256_decimal(&sell_raw) {
                            let sell_quote = encode_quote_exact_input_single(
                                &token_bytes,
                                &weth_bytes,
                                &sell_u256,
                                *fee,
                                    ...
                                    if rev_bytes.len() >= 32 {
                                        ...
                                        if let Ok(rev_amount) = raw_to_amount(&rev_hex, 18) {
                                            ...
                                            if loss_pct > 50.0 {
                                                ...
                                                risk_score += 10;
                                                flags
                                                    .push(format!("Extreme tax: {:.1}%", loss_pct));
