...
pub async fn execute_dex_watch_wallet(
    ...
    let wallet = args["wallet_address"]
        .as_str()
    ...
    match eth_get_balance(rpc_url, addr_clean).await {
        ...
        Err(e) => {
            output.push_str(&format!("ETH Balance: error ({})\n", e));
        }
    }
                ...
                if token_addr.starts_with("0x") && token_addr.len() == 42 {
                    ...
                    if let Ok(result) = eth_call(rpc_url, token_addr, &call_data).await {
                        let symbol = match eth_call(rpc_url, token_addr, &encode_symbol()).await {
                            Ok(s) => decode_abi_string(&s)
                                .unwrap_or_else(|_| token_addr[..10].to_string()),
    ...
    let outgoing_logs = chunked_get_logs(
        ...
        block_num,
        vec![
            Some(serde_json::json!(TRANSFER_EVENT_TOPIC)),
            Some(serde_json::json!(wallet_topic)),
        ],
        500,
    )
    .await;
...
pub async fn execute_dex_whale_transfers(
    ...
    if log_arr.is_empty() {
        output.push_str(&format!(
            "No transfers found in last {} blocks.\n",
            blocks_back
        ));
        return Ok(output);
    }
    ...
    output.push_str(&format!(
        "Total volume: {} {}\n\n",
        format_large_number(total_volume),
        symbol
    ));
    ...
    for (i, t) in transfers.iter().take(20).enumerate() {
        output.push_str(&format!(
            "  {}. {} {} | {} → {} | block {} | tx: {}...\n",
            i + 1,
            format_large_number(t.amount),
            symbol,
            &t.from[..8.min(t.from.len())],
            &t.to[..8.min(t.to.len())],
            t.block,
            if t.tx_hash.len() > 14 {
                &t.tx_hash[..14]
    ...
    for (addr, net) in &accumulation_map {
        if addr.contains("000000000000000000000000000000000") {
            continue;
        }
        if *net > 0.0 {
...
pub async fn execute_dex_top_traders(
    ...
    for log in &log_arr {
        ...
        if from_addr == zero_addr || to_addr == zero_addr {
            continue;
        }
        ...
        buyer.buy_count += 1;
        if block < buyer.first_seen_block {
            buyer.first_seen_block = block;
        }
        if block > buyer.last_seen_block {
            buyer.last_seen_block = block;
        }
        ...
        seller.sell_count += 1;
        if block < seller.first_seen_block {
            seller.first_seen_block = block;
        }
        if block > seller.last_seen_block {
            seller.last_seen_block = block;
        }
    }
    ...
    for p in profiles.values() {
        ...
        let win_indicator = if p.total_bought > 0.0 {
            p.total_sold / p.total_bought
        } else {
            0.0
        };
    ...
    scored.sort_by(|a, b| {
        b.net_pnl_tokens
            .partial_cmp(&a.net_pnl_tokens)
            .unwrap_or(std::cmp::Ordering::Equal)
    });

    output.push_str(&format!(
        "Scanned blocks {} → {} ({} blocks, {} transfers)\n",
        from_block,
        block_num,
        blocks_back,
        log_arr.len()
    ...
    output.push_str(&format!(
        "Unique traders: {} (min {} trades filter)\n\n",
        scored.len(),
        min_trades
    ));

    output.push_str("Top Profit Takers (sold more than bought — realized gains):\n");
    let profit_takers: Vec<&TraderScore> = scored
        .iter()
        .filter(|s| s.net_pnl_tokens > 0.0)
        .take(15)
        .collect();
    if profit_takers.is_empty() {
        ...
        for (i, t) in profit_takers.iter().enumerate() {
            ...
            output.push_str(&format!(
                "     Bought: {} {} | Sold: {} {} | Net: +{} {}\n",
                format_large_number(t.total_bought),
                symbol,
                format_large_number(t.total_sold),
                symbol,
                format_large_number(t.net_pnl_tokens),
                symbol
            ));
            output.push_str(&format!(
                "     Trades: {} | Sell/Buy ratio: {:.2}x | First block: {}\n",
    ...
    let mut accumulators: Vec<&TraderScore> =
        scored.iter().filter(|s| s.still_holding > 0.0).collect();
    accumulators.sort_by(|a, b| {
        b.still_holding
            .partial_cmp(&a.still_holding)
            .unwrap_or(std::cmp::Ordering::Equal)
    });
    ...
    if accumulators.is_empty() {
        ...
        for (i, t) in accumulators.iter().take(15).enumerate() {
            ...
            output.push_str(&format!(
                ...
                format_large_number(t.total_sold),
                symbol,
                format_large_number(t.still_holding),
                symbol
            ));
            output.push_str(&format!(
                "     Trades: {} | First seen: block {}\n",
    ...
    let mut early_winners: Vec<&TraderScore> = scored
        .iter()
        .filter(|s| s.first_block <= early_cutoff && s.net_pnl_tokens > 0.0)
        .collect();
    early_winners.sort_by(|a, b| {
        b.net_pnl_tokens
            .partial_cmp(&a.net_pnl_tokens)
            .unwrap_or(std::cmp::Ordering::Equal)
    });
...
async fn parse_transfer_log(
    ...
    let symbol = match eth_call(rpc_url, &token_addr, &encode_symbol()).await {
        Ok(s) => decode_abi_string(&s)
            .unwrap_or_else(|_| token_addr[..8.min(token_addr.len())].to_string()),
