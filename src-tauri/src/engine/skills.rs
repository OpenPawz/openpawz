...
pub fn get_enabled_skill_instructions(store: &SessionStore) -> Result<String, String> {
    ...
    // Also include enabled community skills (from skills.sh ecosystem)
    let community_instructions = get_community_skill_instructions(store).unwrap_or_default();
...
pub async fn install_community_skill(
    ...
    info!("[skills] Installed community skill: {} from {}", id, source);
...
impl SessionStore {
    ...
    pub fn save_community_skill(&self, skill: &CommunitySkill) -> Result<(), String> {
        let conn = self.conn.lock();
        conn.execute(
            "INSERT INTO community_skills (id, name, description, instructions, source, enabled, installed_at, updated_at)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)
             ON CONFLICT(id) DO UPDATE SET name=?2, description=?3, instructions=?4, source=?5, updated_at=?8",
            rusqlite::params![
                skill.id, skill.name, skill.description, skill.instructions,
                skill.source, skill.enabled as i32, skill.installed_at, skill.updated_at
            ],
    ...
    pub fn list_community_skills(&self) -> Result<Vec<CommunitySkill>, String> {
        let conn = self.conn.lock();
        let mut stmt = conn.prepare(
            "SELECT id, name, description, instructions, source, enabled, installed_at, updated_at
...
/// Get instructions from all enabled community skills (for prompt injection).
pub fn get_community_skill_instructions(store: &SessionStore) -> Result<String, String> {
