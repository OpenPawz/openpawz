...
pub(crate) async fn handle_inbound_message(
    ...
    for msg in messages {
        ...
        debug!(
            ...
            sender_id,
            if text.len() > 50 {
                format!("{}...", &text[..text.floor_char_boundary(50)])
            } else {
                text.to_string()
            }
        );
        ...
        let mut config: WhatsAppConfig =
            match channels::load_channel_config(&app_handle, CONFIG_KEY) {
                Ok(c) => c,
                Err(e) => {
                    error!("[whatsapp] Load config: {}", e);
                    continue;
                }
            };
        ...
        match response {
            ...
            Err(e) => {
                ...
                let _ =
                    send_whatsapp_message(&app_handle, remote_jid, &format!("Error: {}", e)).await;
            }
            _ => {}
        }
    }
